...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const whatsappInquiryUrl = SITE_CONFIG.buildWhatsAppUrl(
    SITE_CONFIG.buildProductInquiryMsg(
      product.name,
      product.sku,
      product.price,
    ),
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-start sm:items-center justify-center p-0 sm:p-4 overflow-hidden animate-in fade-in-0"
      onClick={onClose}
    >
      <div
        id="product-detail-modal-container"
        className="bg-white dark:bg-slate-900 w-full max-w-4xl h-[100dvh] sm:h-auto sm:max-h-[92vh] sm:rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-5 flex flex-col gap-3">
              ...
              <div className="p-3 rounded-xl bg-orange-50/70 dark:bg-orange-950/30 border border-orange-200/60 dark:border-orange-800/40 flex items-center gap-3 text-xs text-orange-900 dark:text-orange-200">
                ...
                <span>
                  {product.warranty ||
                    "Garantía Oficial respaldada por Ferretería July"}
                </span>
              </div>
            </div>
            ...
            <div className="md:col-span-7 flex flex-col justify-between space-y-6">
              <div>
                ...
                {/* Precios */}
                <div className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between">
                  <div>
              ...
              <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                  {/* Selector Cantidad */}
                  <div className="self-start flex items-center border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-800">
                    <button
                ...
                >
                  <MessageSquare className="w-4 h-4 fill-current" /> Consultar
                  disponibilidad y cotización en WhatsApp
                </a>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-4 overflow-x-auto border-b border-slate-200 dark:border-slate-800 mb-6">
              <button
                onClick={() => setActiveTab("details")}
                className={`shrink-0 pb-3 text-xs sm:text-sm font-bold transition-colors relative ${
                  activeTab === "details"
                    ? "text-[#f97316]"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <h4 className="font-bold text-slate-900 dark:text-white mb-2 text-xs uppercase flex items-center gap-1.5">
                      <Truck className="w-4 h-4 text-[#f97316]" /> Despacho y
                      Entrega
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Entregas en almacén central o envíos a obras en Lima
                      Metropolitana en 24 a 48 horas hábiles.
                    </p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <h4 className="font-bold text-slate-900 dark:text-white mb-2 text-xs uppercase flex items-center gap-1.5">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Cambio directo por falla de fabricación durante los
                      primeros 7 días y soporte del servicio técnico oficial de
                      la marca.
                    </p>
                  </div>
                </div>
